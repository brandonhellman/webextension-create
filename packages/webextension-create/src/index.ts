...
inquirer
  .prompt(questions)
  .then((answers: { name: string; template: string }) => {
...
function createPackageJson(extPathToRoot: string, extName: string) {
  console.log();
  console.log(
    `Creating package.json for ${extName} in ${chalk.green(extPathToRoot)}`
  );
  console.log();
  ...
  fs.outputJsonSync(path.join(extPathToRoot, 'package.json'), packageJson, {
    spaces: 2,
  });
}
...
function installScripts(extPathToRoot: string, dev: boolean) {
  ...
  console.log(
    `${command} webextension-scripts in ${chalk.green(extPathToRoot)}`
  );
  console.log();

  spawn.sync('npm', [command, 'webextension-scripts'], {
    stdio: 'inherit',
    cwd: extPathToRoot,
  });
}
...
function initScript(extPathToRoot: string, template: string) {
  ...
  spawn.sync('npx', ['webextension-scripts', 'init', template], {
    stdio: 'inherit',
    cwd: extPathToRoot,
  });
}
